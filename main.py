...
def mesas(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(request, "mesas.html", context(request,db) | {"zonas":db.scalars(select(Zona).order_by(Zona.orden)).all()})
...
def inventario(request:Request,db:Session=Depends(get_db)):
    return templates.TemplateResponse(request, "inventario.html", context(request,db)|{"productos":db.scalars(select(Producto).order_by(Producto.nombre)).all()})
...
def productos(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(request, "productos.html", context(request, db) | {"productos": db.scalars(select(Producto).order_by(Producto.nombre)).all(), "categorias": db.scalars(select(Categoria).order_by(Categoria.nombre)).all()})
...
def empleados(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(request, "empleados.html", context(request, db) | {"empleados": db.scalars(select(Empleado).order_by(Empleado.nombre)).all(), "turnos": db.scalars(select(Turno).order_by(Turno.entrada.desc()).limit(50)).all()})
...
def produccion(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "produccion.html", context(request, db) | {"recetas": recetas, "ordenes": ordenes, "productos": db.scalars(select(Producto).order_by(Producto.nombre)).all(), "insumos": db.scalars(select(Producto).where(Producto.tipo.in_(["insumo", "elaborado"])).order_by(Producto.nombre)).all()})
...
def nomina(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "nomina.html", context(request, db) | {"parametros": db.scalar(select(ParametrosNomina).order_by(ParametrosNomina.vigencia_desde.desc())), "periodos": periodos, "liquidaciones": liquidaciones, "empleados": db.scalars(select(Empleado).where(Empleado.activo == True).order_by(Empleado.nombre)).all()})
...
def compras(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "compras.html", context(request, db) | {"proveedores": db.scalars(select(Proveedor).order_by(Proveedor.nombre)).all(), "productos": db.scalars(select(Producto).where(Producto.activo == True).order_by(Producto.nombre)).all(), "filas": filas})
